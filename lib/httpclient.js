...
var {InputStream, BufferedOutputStream, OutputStreamWriter, BufferedWriter,
        ByteArrayOutputStream, PrintWriter, OutputStreamWriter, BufferedInputStream} = java.io;
 ...
 * @param {java.net.HttpURLConnection} connection The connection
 * @returns The response as ByteArray
 * @type ByteArray
 */
...
/**
 * @class A multipart request text part
 * @param {String} data The data
 * @param {String} charset The charset
 * @param {String} filename An optional file name
 * @returns A newly constructed TextPart instance
 * @constructor
 */
var TextPart = function(data, charset, filename) {
    ...
    this.write = function(name, writer, outStream) {
        ...
        if (data instanceof TextStream) {
            // console.log("--- Sending TextStream", name, data, charset, filename);
            data.copy(new TextStream(outStream, {
                "charset": charset || "utf-8"
            })).close();
            outStream.flush();
        } else {
            // console.log("--- Sending TextPart", name, data, charset, filename);
            writer.append(data);
        }
        writer.append(CRLF).flush();
    };
...
/**
 * @class A multipart request binary part
 * @param {String} data The data
 * @param {String} charset The charset
 * @param {String} filename An optional file name
 * @returns A newly constructed TextPart instance
...
var BinaryPart = function(data, filename) {
     ...
     */
    this.write = function(name, writer, outStream) {
        // console.log("--- Sending BinaryPart", name, data, filename);
        writer.append("Content-Disposition: form-data; name=\"")
                .append(name).append("\"");
        if (filename != null) {
            writer.append("; filename=\"").append(filename).append("\"");
